...
type SetupCurlRunnerModule = {
  ensureCurlDependenciesReady: (force?: boolean) => Promise<void>;
};

async function runSetupCurlRunner(baseDir: string): Promise<void> {
  try {
    const runnerPath = resolve(baseDir, "dist", "update", "setup-curl-runner.js");
    const moduleUrl = pathToFileURL(runnerPath).href;
    const mod = (await import(moduleUrl)) as SetupCurlRunnerModule;
...
async function boot(main: BrowserWindow): Promise<void> {
...
async function safeBoot(main: BrowserWindow): Promise<void> {
  try {
    await boot(main);
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
